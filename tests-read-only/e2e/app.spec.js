...
test.describe("Pagina interraciones", () => {
  ...
  const getDataIds = (elements) =>
    Promise.all(elements.map(async (el) => await el.getAttribute("data-id")));
  ...
  const getSortOptions = async (page) => {
    ...
    if (!sortOrderEl) {
      // simple sort UI with just one type of sort
  ...
  test.describe("filter", () => {
    ...
    test(`cuando elige un filter a otro los resultados se cambian`, async ({
      page,
    }) => {
  ...
  test.describe("sort + filter", () => {
    ...
    test("cuando elige un filtro y un sort, los resultados son afectado de ambos", async ({
      page,
    }) => {
      await selectFilter.selectOption({ index: 1 });
    ...
    test('el button "button-clear" limpia los filtros y sort, volvemos a los resultados original', async ({
      page,
    }) => {
      await page.goto("http://localhost:3000/");
      const originalLis = await page.$$(liSelector);
